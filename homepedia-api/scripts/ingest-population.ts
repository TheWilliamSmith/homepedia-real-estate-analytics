...
async function fetchPopulation(year: number): Promise<WorldBankEntry[]> {
  ...
  do {
    ...
    const res = await fetch(url);
    if (!res.ok)
      throw new Error(`Failed to fetch population page ${page}: ${res.status}`);
...
async function upsertCountryAndPopulation(
  ...
  try {
    ...
    const existing = await prisma.populationData.findUnique({
      where: { countryId_year: { countryId: country.id, year } },
    });

    await prisma.populationData.upsert({
      where: { countryId_year: { countryId: country.id, year } },
...
async function main() {
  ...
  console.log(`\nHomepedia — Population ingestion for year ${year}`);
  ...
  console.log(`Fetching population data (indicator ${INDICATOR})...`);
  const allEntries = await fetchPopulation(year);
  ...
  for (let i = 0; i < realCountries.length; i++) {
    ...
    await upsertCountryAndPopulation(entry, countryMeta, year, stats);
  }
